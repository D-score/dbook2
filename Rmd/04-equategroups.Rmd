...
good equate fit, as presented in the two examples in Figure \@ref(fig:ploteqfit).
...
curve. Also for this equate the fit indices confirm the poor fit and are larger
than 1. Both equates should be deactivated in an updated model.
...
differential item functioning. This means that the items in the equate group are
equally difficult for children in different cohorts. This assumption is critical
for active equate groups. If it is not met, restricting the difficulty
parameters to be equal across cohorts may introduce unwanted bias in comparisons
between cohorts.
...
empirical curves of different colors for each cohort. In both equate groups the
...
In Figure \@ref(fit:ploteqdif), two passive equate groups are displayed, that
show differential item functioning between cohorts. The empirical curves are
displayed for each cohort sepeartely in different colors. The difference between
...
GCDG-ZAF), and for difficult for children in Colombia (blue curve;
...
(yellow and pink curves; GCDG-JAM-LBW and GCDG-JAM-SUNTED) and more dificult for
