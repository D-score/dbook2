...
several steps. By clicking 'Next', the next step will show in the plot. In the
first step the $D$-score is plotted by age and in the subsequent steps, this
relation is modeled in an LMS model. The goal of an LMS model is to describe the
relation of the $D$-score by age with age-conditional z-scores. This is done by
three smoothed curves representing the median (step 2), coefficient of variation
and the skewness. In step 3 the centile lines for this model are presented. The
resulting age-standardized scores for development (DAZ), are plotted in step 4
to 6. The DAZ is used to define on-track development. So $D$-scores within 2
