...
tabtrack$`on-track` <- round(tabtrack$`on-track` * 100, 1)


ft <- flextable(tabtrack)
ft <- set_caption(ft, "Percentage of on-track children per country.")
ft <- set_header_labels(ft, country = "Country", 
                        `on-track` = "On-track (%)")
knit_print(ft)
...
```{r stunt, results = 'hide', fig.keep = 'all',echo=FALSE, fig.width=7, fig.height=7, fig.cap='(ref:stunt)'}
...
tiff("fig/fig7.2.tiff", unit = "in", width = 7, height = 7, res = 300)
ggplot(data = sdg_stunt, aes(y = `Off-track`, x = Stunted, label = ctrycd)) +
  geom_text(size = 5) +
  xlab("Stunting (%)") + ylab("Off-track development (%)")
dev.off()
...
```{r stuntdaz,results = 'hide', fig.keep = 'all', echo=FALSE, fig.width=7, fig.cap= '(ref:stuntdaz)'}
...
tiff("fig/fig7.3.tiff", unit = "in", width = 7, height = 5, res = 300)
ggplot(sdg_stuntdaz, aes(x = stunting, y = DAZ, group = ctrycd))+
  geom_line() + 
  geom_point() +
  xlab("Stunting status")
