...
We order item mappings into five domains: fine motor (FM), gross motor (GM), cognitive (COG), receptive (REC) and expressive (EXP). Figure \@ref(fig:mapping) displays the connections between the instruments via the item groups in each domain. The items are displayes as the leafs and are colored by instrument. The blue lines display the links between the items (and instruments). 
...
#rec -----
indicator <- which(grepl("REC", names(eqlist)))
eqinst1 <- eqinst[indicator]
