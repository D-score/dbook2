...
tiff("fig/fig6.3.tiff", unit = "in", width = 8, height = 5, res = 300)
ggplot(data = dmetric::model_lean$dscore )+
  geom_smooth(aes(x=n, y=sem), formula = y ~ splines::bs(x, 4), se = FALSE, color = mice::mdc(2))+
  xlab("Number of items administered") +
  ylab("Standard error of measurement")
...
tiff("fig/fig6.4.tiff", unit = "in", width = 8, height = 5, res = 300)
ggplot(data = sem_daz_a )+
  geom_point(aes(x=agenum, y=mean_daz)) +
  geom_errorbar(aes(x = agenum, ymin=low_daz, ymax=high_daz), width=.1)+
  xlab("Age (months)") + ylab("DAZ") +
...
tiff("fig/fig6.6.tiff", unit = "in", width = 8, height = 5, res = 300)
  ggplot(data = sem_daz_c, aes(reorder(cohort, sem_pool), sem_pool)) +
  geom_col() + theme(axis.text.x = element_text(angle = 60, hjust = 1))+
    xlab("")+ ylab("Cohort sem")
...
```{r npsem, echo = FALSE}
ft <- flextable(sem_daz_c[,c(1,3,6)])
ft <- set_caption(ft, "Test length and probability to pass the items per cohort.")
ft <- set_table_properties(ft, layout = "autofit", width = .9)
ft <- set_header_labels(ft, cohort = "Cohort", 
                        `test length (median)` = "Test length",
                        `pass probability (median)` = "Pass probability")
knit_print(ft)
...
info_data <-
 ...
 p1 <-ggplot(data = info_wby_domain, aes(x = ability, y = info, fill = domain)) +
...
tiff("fig/fig6.7.tiff", unit = "in", width = 11, height = 5, res = 300)
p1 
dev.off()
...
p1
tiff("fig/fig6.8.tiff", unit = "in", width = 11, height = 5, res = 300)
p1
dev.off()
...
cortab <- data.frame("names" = rownames(cortab), round(cortab, 2))
ft <- flextable(cortab)
ft <- set_caption(ft, "Pearson correlation of the DAZ and five domain-specific DAZ scores.")
ft <- set_header_labels(ft,  DAZ = "DAZ", names = " ")
knit_print(ft)
...
```{r domaindex,results = 'hide', fig.keep = 'all', echo=FALSE, fig.cap='(ref:domaindex)', fig.keep='all', results='hide', fig.width=11, fig.heiht = 3}
...
p1
tiff("fig/fig6.9.tiff", unit = "in", width = 11, height = 3, res = 300)
p1
dev.off()
