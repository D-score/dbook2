...
(ref:dscoredist) D-score distributions by study. Click [ZOOM](https://d-score.org/dbook-apps/gcdgdscores/#display=by_cohort&nrow=1&ncol=1&arr=row&pg=6&labels=cohort&sort=cohort;asc&filter=&sidebar=&fv=) to enlarge the image.
...
(ref:dazdist) DAZ distributions by study. Click [ZOOM](https://d-score.org/dbook-apps/gcdgdaz/#display=by_cohort&nrow=1&ncol=1&arr=row&pg=6&labels=cohort&sort=cohort;asc&filter=&sidebar=&fv=) to enlarge the image.

Figure \@ref(fig:dazdist) plots the same data with D-score transformed into DAZ scores. Replacing the D-score by the DAZ emphasises the differences both within and between studies. The majority of observations lies between the -2 SD and +2 SD lines in all cohorts. It is easier to spot deviating trends, e.g., for the Jamaican or Ethiopian data.
...
(ref:dscoresems) The standard error of measurement ($sem$) around the age-standardized D-scores (DAZ) per cohort. Click [ZOOM](https://d-score.org/dbook-apps/gcdgsem/#display=by_cohort&nrow=1&ncol=1&arr=row&pg=6&labels=cohort&sort=cohort;asc&filter=&sidebar=&fv=) to enlarge the display.
