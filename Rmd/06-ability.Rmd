...
child development: Fine Motor, Gross Motor, Expressive, Receptive, Cognitive. The
...
that we need to know what items were adminstered to measure the D-score and for
these items we need to know to which domains the items are linked and what
relative information the items provide for the calculated D-score.

For example below you see the data for a child that is 1,5 years old and as part
of the .. cohort has scores on by3 items. Table \@ref(tab:example_part1) shows
the responses to the by3 items that are in the gcdg model in the **Score**
column. This child has a score of 57.12$D$. For each item experts have indicated
...
For each item we can calculate the relative information that it contributes tot
he $D$-score of 57.12. In [the paragraph "Item information" in
dbook1](https://stefvanbuuren.name/dbook1/sec-iteminformation.html) we described
how to calculate the item information for an ability. We can combine the
porportional domain loading to the relative information that the item provides
...
The information that is contributed by each item can be summed and that way we
get what information for each domain was used to get to the $D$-score for this
child. For this example about 50% of the information was Cognitive, about 40%
was Fine motor and additionally partly adaptive and partly receptive information
was used.
