...
The domain information for an individual $D$-score can also be calculated. To do that we need to know what items were adminstered to measure the D-score and for these items we need to know to which domains the items are linked and what relative information the items provide for the calculated D-score.  

For example below you see the data for a child that is 1,5 years old and as part of the .. cohort has scores on by3 items. Table \@ref(tab:example_part1) shows the responses to the by3 items that are in the gcdg  model in the **Score** column. This child has a score of 57.12$D$. For each item experts have indicated to what domain(s) the item is linked. Items can link to multiple domains and based on expert voting, we have a proportional loading for each item to a domain.  
...
For each item we can calculate the relative information that it contributes tot he $D$-score of 57.12. In [the paragraph "Item information" in dbook1](https://stefvanbuuren.name/dbook1/sec-iteminformation.html) we described how to calculate the item information for an ability. We can combine the porportional domain loading to the relative information that the item provides to the $D$-score of the child. In Figure \@ref(fig:example_part2)  this is displayed.
...
plot_data$item <- factor(plot_data$item, levels = unique(plot_data$item[order(plot_data$rel_info)]))

ggplot(data = plot_data, aes(x = item, y = contribution, color = domain, fill = domain)) +
