...
Using the same methodology as described above, we can extract the domain information for an individual D-score. For that we need to know what items were adminstered to obtain the D-score. For these items we need to know to which domain(s) the item is linked and what relative information the item provides for the calculated D-score.
...
subject <- unique(gcdg_lean$itm$subjid) [sample(25410, 2)]
...
ggplot(domain_contr, aes(fill=domain, y=(d*proportion), x=(a*12))) + 
    geom_bar(position="stack", stat="identity")+
  xlab("Age")+ylab("D-score")+
  scale_colour_manual(values = domain_colors)

domain_contr_i <- domain_contr %>% filter(subjid == 4702253)
