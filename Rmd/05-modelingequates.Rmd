...
<!---total number of observed scores is excluding the missings, right? Then the sentence "While this... " does not fit, maybe change to: "While the total matrix with child-visits and items has over 95 million cells, about 97 percent of these entries in the matrix is missing."--->

Subject matter experts classified items in groups of similar appearing items, thus forming 184 equate groups. Each equate group contains at least two items. 
...
Figure \@ref(fig:model1339) is a display of the D-score by age for all 16 cohorts under four models. As a rough reference to compare, the gray curves in the back represent the Dutch model as calculated from the [SMOCC study](https://stefvanbuuren.name/dbook1/sec-smoccstudy.html). In order to speed up the calculations, the figure shows a random subsample of 25\% of all points. Manipulate the plot controls to switch cohorts.
...
```{r itemfit, echo=FALSE, fig.cap='(ref:itemfit)', fig.height=5, warning=FALSE}
...
```{r itembox, echo=FALSE, fig.cap='(ref:itembox)', fig.height=5., warning=FALSE}
...
It is not clear why `bat` does not fit. Perhaps the scoring system of the Battelle in three categories invokes scoring behaviour that is different from the PASS/FAIL scoring used by most other instruments, even though this appears to be less of a troublesome aspect in ASQ-3, which also uses three response categories.
...
There is no single index of model fit that will detect all possible disturbances. modelling equates is more like a balancing act among multiple competing objectives, such as
